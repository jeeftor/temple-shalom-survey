 ...
 * With ?key=TOKEN&rows=N, returns the last N rows from the Sheet.
 */
function doGet(e) {
  ...
  if (params.key && params.rows) {
    ...
    return jsonResponse(getRecentRows(parseInt(params.rows, 10) || 10));
  }
...
/**
 * Diagnostic function for clasp run — returns last N rows from the Sheet.
 * Usage: clasp run getRecentRows
 */
function getRecentRows(maxRows) {
  try {
    const ss = SpreadsheetApp.openById(SHEET_ID);
    ...
    if (lastRow === 0) return { rows: [], count: 0 };
    const count = Math.min(lastRow - 1, maxRows || 10);
    if (count <= 0) return { totalRows: lastRow, columns: [], recent: [] };
    const headers = sheet.getRange(1, 1, 1, lastCol).getValues()[0];
    const data = sheet.getRange(lastRow - count + 1, 1, count, lastCol).getValues();
    return {
      totalRows: lastRow,
      columns: headers,
      recent: data.map(row => {
